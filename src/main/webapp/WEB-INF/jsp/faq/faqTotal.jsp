...
<div id='faqmain'>	
	...
	<form name='frm_faqmain' id='frm_faqmain' method='post' >
		<div>
      	...
      	<div>
			...
			<input type="button" id='btnFind1' class="form-control" value='Search'><br>
		</div>   
	</form>
	...
	<ul class="pagination" id="user_pagination">
        <li><a class="${(page.startPage > 1)? '' : 'btnHide' }" id="btnPrev" onclick="guideGoPage1(${page.startPage-1})">«</a></li>

      	<c:forEach var="i" begin="${page.startPage }" end="${page.endPage }">
        	<li class="${(param.nowPage == i)? 'active':'' }"><a id="btnCenter" onclick="guideGoPage1(${i})">${i }</a></li>
        </c:forEach>

       	<li><a class="${(page.endPage < page.totPage)? '' : 'btnHide' }" id="btnNext" onclick="guideGoPage1(${page.endPage+1})">»</a></li>             
    </ul>
