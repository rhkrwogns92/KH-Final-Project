...
<div id='faqmain'>
	...
	<form name='frm_faqmain' id='frm_faqmain' method='post' >
		<div>
      	...
      	<div>
			...
			<input type="button" id='btnFind7' class="form-control" value='Search'><br>
		</div>   
	</form>
	...
	<%-- ─────────────────────────────────── 페이징 부분 ─────────────────────────────────── --%>
        ...
        <li><a class="${(page.startPage > 1)? '' : 'btnHide' }" id="btnPrev" onclick="guideGoPage7(${page.startPage-1})">«</a></li>

      	<c:forEach var="i" begin="${page.startPage }" end="${page.endPage }">
        	<li class="${(param.nowPage == i)? 'active':'' }"><a id="btnCenter" onclick="guideGoPage7(${i})">${i }</a></li>
        </c:forEach>

       	<li><a class="${(page.endPage < page.totPage)? '' : 'btnHide' }" id="btnNext" onclick="guideGoPage7(${page.endPage+1})">»</a></li>             
    </ul>
